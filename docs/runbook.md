...
| `VM_UPDATE_REPOSITORY` | `han-cczu/vps-monitor` | 「检查更新」查询的公开 GitHub 仓库，格式 `owner/repo`。只读公开发布信息，不下载也不安装；填错或不可达时只有这张卡片报检查失败，不影响监控、订阅和升级流程 | 22 |
...
入口只有「设置 → 站点」顶部的「版本与更新」卡片一处（2026-09-17 起：节点页不再重复挂同一张卡片，那页只保留各节点的 Agent 版本、逐台升级和批量升级）。点「检查更新」后卡片列出当前面板版本、面板携带的探针版本、更新源的最新稳定版、检查时间和发布说明链接，并分别给出面板与探针的结论。

- **只做检查**：不下载、不安装、不重启，也不向节点下发任何升级。面板升级仍走上面的镜像部署流程，探针升级仍在节点页逐台或批量点「更新 Agent」。
- **版本规则**：更新源只认 `vX.Y.Z` 形式的稳定 Release；草稿、预发布（`-rc`）和其他标签（例如 sing-box 的 `sing-box-v…`）都不计。面板版本或探针版本是 `dev`、自定义后缀（例如 `v0.2.0-observe21`）时不能自动比较，卡片显示「无法自动比较」，这类版本本来也不能自动升级。
- **先有发布才有结论**：公开仓库里必须有打了 `vX.Y.Z` 标签的 Release，检查才可能报「有新版本」。仓库当前没有任何 Release 时，卡片显示「更新源还没有稳定版本发布」，这是预期结果，不是故障。发布面板与探针共用同一个版本号序列，最新稳定版对两者是同一个号。
- **面板携带的探针**来自 `{VM_DATA_DIR}/agent/VERSION`（镜像里是 `/app/agent-dist/VERSION`）。更新源已有更高探针、但面板还带着旧产物时，卡片会提示先部署携带新探针的面板版本——此时节点页不会出现「可更新」，因为面板手里没有可下发的二进制。
- **缓存与限流**：检查成功缓存 10 分钟、失败缓存 1 分钟，期间重复点击复用结果，卡片会写明「本次点击复用了缓存」。未认证的 GitHub API 限额是每 IP 每小时 60 次，面板按缓存最多每小时 6 次左右，正常不会触发限流。
- **失败不会伪装成最新版**：检查失败时卡片显示原因，并保留上一次成功查到的版本（标注「上次成功结果」）。常见原因是面板主机到 `api.github.com` 不通（国内机房常见）、被限流（403/429）、`VM_UPDATE_REPOSITORY` 写错或仓库非公开、发布记录超过 300 条。这些都不影响监控、订阅与升级流程；确认网络受限时可把 `VM_UPDATE_REPOSITORY` 指向可达的镜像仓库，检查器本身只读公开发布信息。

### 设置、生效时间和审计

“设置 → 站点”配置站点标题、IANA业务时区、1000/1024显示单位、四类保留期、告警冷却、订阅统计口径。保存是白名单局部更新，不覆盖订阅模板或其他后台配置；设置与审计在同一事务中提交，失败整体回滚。

- 标题显示在面板顶栏。内存、磁盘容量通过 `formatBytes` 跟随单位设置，1024 显示 KiB/MiB/GiB；流量与网速通过 `formatTrafficBytes`/`formatRate` 固定使用十进制 KB/MB/GB/TB，不改账本原始字节。节点与订阅套餐也按十进制填写；旧版节点填写的 500 GB 实际保存为 536,870,912,000 字节，升级后需将该套餐重新填写为 500 GB 并保存，才会变为 500,000,000,000 字节。
