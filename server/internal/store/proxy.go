...
const subscriberFields = `id,name,note,enabled,auto_disabled,traffic_limit,traffic_used,reset_day,period_start,expire_at,created_at,updated_at,kind`

func scanSubscriber(row scanner, detail bool) (*Subscriber, error) {
	s := &Subscriber{AssignedInbounds: []Assignment{}}
	args := []any{&s.ID, &s.Name, &s.Note, &s.Enabled, &s.AutoDisabled, &s.TrafficLimit, &s.TrafficUsed, &s.ResetDay, &s.PeriodStart, &s.ExpireAt, &s.CreatedAt, &s.UpdatedAt, &s.Kind}
	if detail {
