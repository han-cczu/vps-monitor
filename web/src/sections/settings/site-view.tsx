  ...
  return (
    <Card sx={{ p: { xs: 2, md: 3 } }}>
      <Stack spacing={3} component="form" onSubmit={save}>
        ...
        {(error || message) && <Alert severity="error">{message || getErrorMessage(error)}</Alert>}
          ...
          <>
            ...
            >
              ...
              <TextField
                ...
                onChange={(e) => change('enforce.count_mode', e.target.value as 'sum' | 'download')}
            ...
            <Alert severity="info">缩短保留期后，下一次清理会删除超期历史；请按需要先备份。</Alert>
            <Button
              type="submit"
              variant="contained"
              loading={busy}
